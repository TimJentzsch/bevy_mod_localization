...
use super::path::{get_base_path, normalize_path};
...
///
/// HACK: More or less copied from `FileWatcher` from Bevy.
pub struct LocalizedFileWatcher {
    _watcher: Debouncer<RecommendedWatcher, FileIdMap>,
}

impl LocalizedFileWatcher {
    pub fn new(
        root: PathBuf,
        sender: Sender<AssetSourceEvent>,
        debounce_wait_time: Duration,
    ) -> Result<Self, notify::Error> {
        let root = normalize_path(get_base_path().join(root).as_path());
        ...
        Ok(LocalizedFileWatcher { _watcher: watcher })
    }
}

impl AssetWatcher for LocalizedFileWatcher {}
...
pub(crate) fn new_asset_event_debouncer(
    ...
    let root = get_base_path().join(root);
